...
function New-MemoryMappedFile
{
    ...
    [OutputType([IO.MemoryMappedFiles.MemoryMappedFile])]
	param(
        ...
        [ValidateNotNullOrEmpty()]
        [Alias('access')]
        [IO.MemoryMappedFiles.MemoryMappedFileAccess] $MemoryMappedFileAccess,
        ...
        [ValidateNotNullOrEmpty()]
        [Alias('options')]
        [IO.MemoryMappedFiles.MemoryMappedFileOptions] $MemoryMappedFileOptions,
        ...
        [IO.HandleInheritability] $HandleInheritability,
        ...
        if ($pscmdlet.ShouldProcess($Name, $MyInvocation.MyCommand.Name))
        {
            try
            {
                ...
                if ($PSCmdlet.ParameterSetName -eq 'Default')
                {
                    if ($CreateOrOpen)
                    {
                        [IO.MemoryMappedFiles.MemoryMappedFile]::CreateOrOpen($Name, $Size)

                    }
                    else
                    {
                        [IO.MemoryMappedFiles.MemoryMappedFile]::CreateNew($Name, $Size)
                ...
                if ($PSCmdlet.ParameterSetName -eq 'MemoryMappedFileAccessSet')
                {
                    if ($CreateOrOpen)
                    {
                        [IO.MemoryMappedFiles.MemoryMappedFile]::CreateOrOpen($Name, $Size, $MemoryMappedFileAccess)

                    }
                    else
                    {
                        [IO.MemoryMappedFiles.MemoryMappedFile]::CreateNew($Name, $Size, $MemoryMappedFileAccess)
                ...
                if ($PSCmdlet.ParameterSetName -eq 'MemoryMappedFileOptionSet')
                {
                    if ($CreateOrOpen)
                    {
                        [IO.MemoryMappedFiles.MemoryMappedFile]::CreateOrOpen($Name, $Size, $MemoryMappedFileAccess, $MemoryMappedFileOptions, $HandleInheritability)

                    }
                    else
                    {
                        [IO.MemoryMappedFiles.MemoryMappedFile]::CreateNew($Name, $Size, $MemoryMappedFileAccess, $MemoryMappedFileOptions, $HandleInheritability)
                ...
                {
                    [IO.MemoryMappedFiles.MemoryMappedFile]::CreateFromFile($Path)
                ...
                {
                    [IO.MemoryMappedFiles.MemoryMappedFile]::CreateFromFile($Path, $FileMode)
                ...
                {
    	            [IO.MemoryMappedFiles.MemoryMappedFile]::CreateFromFile($Path, $FileMode, $Name)
                ...
                {
    	            [IO.MemoryMappedFiles.MemoryMappedFile]::CreateFromFile($Path, $FileMode, $Name, $Size)
                ...
                {
                    [IO.MemoryMappedFiles.MemoryMappedFile]::CreateFromFile($Path, $FileMode, $Name, $Size, $MemoryMappedFileAccess)
                ...
                {
    	            [IO.MemoryMappedFiles.MemoryMappedFile]::CreateFromFile($FileStream ,$Name, $Size, $MemoryMappedFileAccess, $HandleInheritability, $leaveOpen)
...
function Open-MemoryMappedFile
{
	...
	param(
        ...
        [IO.MemoryMappedFiles.MemoryMappedFileRights] $MemoryMappedFileRights,
    ...
    Process
    {
	    ...
	    if ($PSCmdlet.ParameterSetName -eq 'Default') {	[IO.MemoryMappedFiles.MemoryMappedFile]::OpenExisting($Name) }

        if ($PSCmdlet.ParameterSetName -eq 'MemoryMappedFileRights') {	[IO.MemoryMappedFiles.MemoryMappedFile]::OpenExisting($Name, $MemoryMappedFileRights) }

        if ($PSCmdlet.ParameterSetName -eq 'HandleInheritability') {	[IO.MemoryMappedFiles.MemoryMappedFile]::OpenExisting($Name, $MemoryMappedFileRights, $HandleInheritability) }
...
function Out-MemoryMappedFile
{
	...
	param(
        ...
        [ValidateNotNull()]
        [ValidateNotNullOrEmpty()]
	    [IO.MemoryMappedFiles.MemoryMappedFile]$MemoryMappedFile,
    ...
    Process
    {
        if ($PsCmdlet.ParameterSetName -eq 'Default')
        {
            if ($PsCmdlet.ShouldProcess($String, $MyInvocation.MyCommand.Name))
            {
                try
                {
...
function Read-MemoryMappedFile
{
    ...
    Process
    {
        if ($PsCmdlet.ParameterSetName -eq 'Default')
        {
            if ($PsCmdlet.ShouldProcess($MemoryMappedFile, $MyInvocation.MyCommand.Name))
            {
                try
                {
	                $Stream = $MemoryMappedFile.CreateViewStream()
...
function Save-MemoryMappedFile
{
    ...
    Process
    {
        if ($PsCmdlet.ParameterSetName -eq 'Default')
        {
            ...
            {
                $result = [IO.File]::Exists($Path)

                if ($result -eq $true)
                {
                    Write-Output -InputObject ('[{0}] overrides the file [{1}].' -f $MyInvocation.MyCommand.Name, $Path)
                }
            }
        }
    }
